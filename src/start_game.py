...
class People(pygame.sprite.Sprite):  # 继承动画精灵的类
    ...
    def move(self):
        ...
        if (
            ...
            or self.rect.right >= screen.get_rect().right
        ):
...
class ScoreBoard:
    def __init__(self, screen):
        ...
        # self.stats = stats
        # 分数图形的参数
        self.text_color = (0x66, 0xCC, 0xFF)  # 字体颜色
...
class Countdown:
    ...
    def prep_time(self):
        str_time = (
            str(int(self.time / 60)) + ":" + ("0" + str(int(self.time) % 60))[-2:]
        )  # 转化成MM:SS字符串
...
while running:
    ...
    if pygame.key.get_pressed()[pygame.K_SPACE]:
        if not (
            m.grid[int(my_people.rect.top / 32)][int(my_people.rect.left / 32)] == 1
        ):  # 进行传送
            new_point = m.gate[
                int(my_people.rect.top / 32), int(my_people.rect.left / 32)
            ]
    ...
    if pygame.key.get_pressed()[pygame.K_UP]:
        if m.grid[int(my_people.rect.top / 32) - 1][
            int(my_people.rect.left / 32)
        ]:  # y轴的方向是向下的
    ...
    if (
        m.score[int(my_people.rect.top / 32)][int(my_people.rect.left / 32)] == 1
    ):  # 吃铜锣烧
    ...
    screen.fill([0, 0, 0])
    for i in range(
        math.floor((my_people.rect.top) / 32) - 5,
        math.floor((my_people.rect.top) / 32) + 6,
    ):
        # try:
        if i >= 0 and i < 20:
            for j in range(
                math.floor((my_people.rect.left) / 32 - 5),
                math.floor((my_people.rect.left) / 32) + 6,
            ):
                if j >= 0 and j < 40:
                    if m.grid[i][j] == 1:
